...
class VisibilityIndexer extends AbstractIndexer {
    private wpdb $db;
    ...
    public function __construct() {
        global $wpdb;
        $this->db      = $wpdb;
        $this->table   = $wpdb->prefix . 'product_visibility_index';
        $this->staging = $wpdb->prefix . 'product_visibility_index_staging';
        $this->logger  = new Logger();
        $this->cache   = new CachePurger();
        $this->snapshotEnabled = Config::snapshotV2Enabled();
        if ( $this->snapshotEnabled ) {
            $this->snapshotWriter = new SnapshotWriter( 'visibility', new SnapshotVersionManager() );
        }
    }
    ...
    public function processBatch( array $jobs ) : void {
        ...
        $this->logger->info( 'visibility', 'Indexed visibility batch', [ 'count' => count( $rows ), 'products' => $productIds, 'version' => $version ] );
        update_option( 'aurora_last_rebuild_visibility', current_time( 'mysql', true ), false );
    ...
    private function processSnapshotBatch( array $productIds ) : void {
        ...
        $this->logger->info( 'visibility', 'Indexed visibility snapshot batch', [
            'count'    => $result['count'],
        ...
        foreach ( $rows as $row ) {
            $this->db->insert(
                ...
                [
                    ...
                    'catalog_flags'=> $row['catalog_flags'],
    ...
    private function swapBatch( string $batchId, array $productIds ) : void {
        ...
        $this->db->query(
            $this->db->prepare(
                "INSERT INTO {$this->table} (product_id, variation_id, sku, visibility, catalog_flags, channel_mask, version, updated_at)
                 SELECT product_id, variation_id, sku, visibility, catalog_flags, channel_mask, version, %s
                 FROM {$this->staging}
                 WHERE batch_id = %s",
